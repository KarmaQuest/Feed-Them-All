...
export default function BadgesSection() {
  ...
  const [modal, setModal] = useState<null | {
    mode: 'create' | 'edit'
    data: typeof EMPTY_BADGE_FIELDS
    cond: ConditionForm
    id?: string
  }>(null)
  ...
  async function handleSave() {
    ...
    try {
      if (modal.mode === 'create') {
        ...
        setBadges(prev =>
          prev.map(b => (b.id === modal.id ? ({ id: modal.id!, ...payload } as AdminBadge) : b))
        )
      }
      closeModal()
    } finally {
        ...
        <table className="admin-table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr className="loading-row">
                <td colSpan={5}>Aucun badge</td>
              </tr>
            ) : (
              badges.map(b => (
                <tr key={b.id}>
                  <td>
                    <code style={{ color: '#818cf8' }}>{b.slug}</code>
                  </td>
                  ...
                  <td style={{ display: 'flex', gap: '0.5rem' }}>
                    <button className="btn btn--style-green btn--sm" onClick={() => openEdit(b)}>
                      Modifier
                    </button>
                    <button
          ...
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-form-grid">
              ...
              <input
                ...
                onChange={e =>
                  setModal({ ...modal, data: { ...modal.data, slug: e.target.value } })
                }
              />
              ...
              <input
                className="inline-input"
                placeholder="ex: Nourrisseur confirme"
                value={modal.data.label}
                onChange={e =>
                  setModal({ ...modal, data: { ...modal.data, label: e.target.value } })
                }
              />
              ...
              <select
                ...
                value={modal.cond.type}
                onChange={e =>
                  setModal({
                    ...modal,
                    cond: {
                      ...modal.cond,
                      type: e.target.value as 'xp_threshold' | 'action_count',
                    },
                  })
                }
              >
              ...
              {modal.cond.type === 'action_count' && (
                <>
                  ...
                  <input
                    className="inline-input"
                    placeholder="ex: feed, report_animal"
                    value={modal.cond.action}
                    onChange={e =>
                      setModal({ ...modal, cond: { ...modal.cond, action: e.target.value } })
                    }
                  />
                </>
              )}
              ...
              <input
                ...
                onChange={e =>
                  setModal({ ...modal, cond: { ...modal.cond, value: e.target.value } })
                }
              />
            </div>
            ...
            >
              JSON :{' '}
              <code style={{ color: '#818cf8' }}>{JSON.stringify(condFormToJSON(modal.cond))}</code>
              {modal.cond.type === 'action_count' && (
                <p style={{ marginTop: '0.4rem', color: '#6b7280', margin: '0.3rem 0 0' }}>
                  Badges cumulables : creer plusieurs badges avec la meme action et des valeurs
                  differentes (ex: feed x10, feed x50, feed x100).
                </p>
              )}
            </div>
